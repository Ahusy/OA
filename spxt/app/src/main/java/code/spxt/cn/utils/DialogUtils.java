...
import code.spxt.cn.view.MyAddressView;
...
public class DialogUtils {
    ...
    private static Object minuPicker;
    ...
    public static void showAddressDialog(Context context, int position, DialogAddressClickListenner dialogAddressClickListenner) {
        ...
        MyAddressView addressView = view.findViewById(R.id.address);
        addressView.setOnAddressPickerSure((address, provinceCode, cityCode) -> {
...
//        });
    ...
    public static void showOnlyDatePicker(Context context, DialogOnlyDatePickerListener listener) {
        View view = LayoutInflater.from(context).inflate(R.layout.dialog_time, null);
        dialog = new AlertDialog.Builder(context).setView(view).create();
        if (dialog.getWindow() != null) {
            dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        ...
        picker.init(year, month, date, (view1, year1, monthOfYear, dayOfMonth) -> {
            int months = monthOfYear + 1;
                listener.onClick(year1 + "-" + months + "-" + dayOfMonth);
                closeDialog();
        });
        dialog.setCancelable(true);
        dialog.show();
    ...
    public interface DialogOnlyDatePickerListener {
        void onClick(String date);
    }
...
//            window.setGravity(Gravity.BOTTOM);
    ...
    public static void showHomeDialog(Context context,String num) {
        View view = LayoutInflater.from(context).inflate(R.layout.dialog_home, null);
        dialog = new AlertDialog.Builder(context).setView(view).create();
        if (dialog.getWindow() != null) {
            dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        }

        ImageView shutDown = view.findViewById(R.id.shut_down);
        TextView count = view.findViewById(R.id.count);
        TextView sure = view.findViewById(R.id.sure);
        if (!TextUtils.isEmpty(num)){
            count.setText(num);
        }
        shutDown.setOnClickListener(v -> closeDialog());
        sure.setOnClickListener(v -> {
            context.startActivity(new Intent(context, MyReviewActivity.class));
            closeDialog();
