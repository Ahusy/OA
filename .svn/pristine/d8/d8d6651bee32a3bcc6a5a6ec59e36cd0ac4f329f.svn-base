...
public class ApprovalFlowView extends View {
    ...
    private int dateHeight = dp2px(30);
    ...
    private void drawTime(Canvas canvas) {
        if (currentOrientation == HORIZONTAL) {
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                float y = getHeight() - dp2px(10);
                paintTwo.getTextBounds(time, 0, time.length(), fontRect);
                int fontH = fontRect.height();
                canvas.drawText(time.split(" ")[0], x, y - fontH - dp2px(3), paintTwo);
                canvas.drawText(time.split(" ")[1], x, y, paintTwo);
            }
        }
