...
public class CardUseActivity extends ToolBarActivity {
    ...
    public static void startAct(Activity activity, BillPublicCarDetail ba, int requestCode) {
        Intent intent = new Intent(activity, CardUseActivity.class);
    ...
    private void initView() {
        ...
        BillPublicCarDetail ab = getIntent().getParcelableExtra(EXTRA_OBJECT);
        if (ab != null) {
            model.setText(ab.getCarType());
            num.setText(ab.getPlateNumber());
            reason.setText(ab.getReason());
            address.setText(ab.getDestination());
            person.setText(ab.getTogether());
            driveName.setText(ab.getDriver());
